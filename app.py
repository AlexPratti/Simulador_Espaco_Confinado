...
with col_esquerda:
    ...
    st.selectbox("1. Selecione o serviço a ser realizado:", servicos_disponiveis, index=None, placeholder="Escolha a atividade...", on_change=resetar_jogo, key="select_servico")
...
with col_direita:
    ...
    if not st.session_state.servico_selecionado or not st.session_state.acidente_selecionado:
        ...
        if st.session_state.erro_procedimento:
            ...
            if st.button("Simular Nova Atividade 🔄", use_container_width=True):
                resetar_jogo()
                st.rerun()

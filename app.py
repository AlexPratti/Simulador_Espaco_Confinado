...
def exibir_imagem_repositorio(nome_arquivo, fallback_texto):
    if os.path.exists(nome_arquivo):
        ...
        # Cria uma caixinha cinza que substitui a imagem temporariamente
        st.markdown(
            f"""
...
with col_esquerda:
    ...
    # Seleção dos parâmetros da simulação
    st.selectbox("1. Selecione o serviço a ser realizado:", servicos_disponiveis, index=None, placeholder="Escolha uma atividade...", on_change=resetar_jogo, key="select_servico")
...
with col_direita:
    ...
    if not st.session_state.servico_selecionado or not st.session_state.acidente_selecionado:
        ...
        # CONTROLE DE TRATAMENTO DE ERROS COM CORREÇÃO DA ETAPA ATUAL ATIVADA
        if st.session_state.erro_procedimento:
            ...
            def avaliar_dupla(tipo_clique, valor):
                if tipo_clique == "quem":
                    ...
                    if st.session_state.responsavel_selecionado is None:
                        st.warning("⚠️ Violação de Procedimento! Você deve escolher o profissional responsável (Passo 1) antes de acionar o dispositivo.")
                        return
...
# ⚙️ 3. Sistemas Atmosféricos e Coletivos
            ...
            with epi2:
                exibir_imagem_repositorio("Radio_Comunicacao.png", "Aparelho HT")
                if st.button("Iniciar Posto de Comunicação", key="b_com", use_container_width=True): avaliar_dupla("o_que", "Comunicacao")
            with epi3:
                exibir_imagem_repositorio("Lado_de_fora.png", "Posto Externo do Vigia")
            ...
            if st.session_state.etapa_atual == (len(fluxo_seguranca) - 1):
                ...
                with res1:
                    exibir_imagem_repositorio("Respirador_Autonomo.png", "Cilindro de Ar")
                    st.caption("Respirador Autônomo")
                with res2:
                    exibir_imagem_repositorio("Colar_Cervical.png", "Imobilizador de Coluna")
                    st.caption("Colar Cervical + Maca Rígida")
                with res3:
                    exibir_imagem_repositorio("Maca_Sked.png", "Maca de Arrasto")
                    st.caption("Maca Sked (Içamento Combinado)")
                with res4:
                    exibir_imagem_repositorio("Talas_Ataduras.png", "Kit Primeiros Socorros")

...
if "missao_abortada" not in st.session_state:
    st.session_state.missao_abortada = False
...
def exibir_imagem_repositorio(nome_arquivo, fallback_texto, largura=None):
    if os.path.exists(nome_arquivo):
        if largura:
            st.image(nome_arquivo, width=largura)
        else:
            st.image(nome_arquivo, use_container_width=True)
    else:
        st.markdown(
            f"""
            <div style="background-color: #262730; border: 2px dashed #4b4d5a; border-radius: 8px; padding: 20px; text-align: center; color: #a1a1a1; font-weight: bold; margin-bottom: 10px;">
...
# Vinculação Condicional de Riscos (Evita misturar riscos irrelevantes com os cenários)
RISCOS_POR_CENARIO = {
    "NR-33": [
        ...
        "Queimadura Química por Contacto Respiratório",
        "Choque Elétrico por Equipamento Não Aterrado"
    ],
    "NR-35": [
        "Queda de Altura com Fratura Exposta no Fêmur",
        "Queda Livre com Suspensão Inerte (Síndrome do Arnês)",
        "Queda de Altura com Trauma na Coluna",
        "Prensamento de Membro com Hemorragia Grave"
    ]
}
...
def obter_fluxo_nr33():
    ...
    return [
        ...
        {"acao": "Montar e preparar a estrutura de ancoragem e o sistema de vantagem mecânica sobre o acesso", "quem_correto": "Vigia", "o_que_correto": "Tripe", "motivo": "Na phase preparatória preventiva, cabe ao Vigia estruturar os sistemas de movimentação externos."},
...
def obter_fluxo_nr35():
    ...
    return [
        {"acao": "Elaborar e validar Análise Preliminar de Risco (APR) listando os perigos da atividade em altura", "quem_correto": "Supervisor", "o_que_correto": "APR", "motivo": "O Supervisor deve garantir que a análise de risco e os recursos de segurança estejam corretos antes da subida."},
...
with col_esquerda:
    ...
    if modulo == "NR-33 (Espaço Confinado)":
        ...
        # Filtra os riscos para exibir apenas os da NR-33
        st.selectbox("2. Escolha o sinistro potencial:", RISCOS_POR_CENARIO["NR-33"], index=None, placeholder="Selecione...", on_change=resetar_jogo, key="sel_a_33")
    ...
    # 2. Fluxo Visual Exclusivo se a Aba selecionada for NR-35
        ...
        # Filtra os riscos para exibir apenas os da NR-35 (removendo asfixia/gases)
        st.selectbox("2. Escolha o risco à saúde associado:", RISCOS_POR_CENARIO["NR-35"], index=None, placeholder="Selecione...", on_change=resetar_jogo, key="sel_a_35")
...
with col_direita:
    ...
    # Validação do 4º Erro Crítico (Abortar Missão)
    elif st.session_state.total_erros >= 4 or st.session_state.missao_abortada:
        st.session_state.missao_abortada = True
        fluxo_ativo = obter_fluxo_nr33() if modulo == "NR-33 (Espaço Confinado)" else obter_fluxo_nr35()
        passo_falho = fluxo_ativo[st.session_state.etapa_atual]
        
        st.error("🚫 MISSÃO ABORTADA CRITICAMENTE! O limite de 4 desvios operacionais foi atingido.")
        st.markdown(
            f"""
            <div style="background-color: #4d1c1c; padding: 15px; border-radius: 5px; border-left: 5px solid #ff4b4b;">
                <strong>Ponto de Interrupção da Linha de Vida:</strong><br>
                ❌ <em>Ação pendente: {passo_falho['acao']}</em><br><br>
                <strong>Gabarito de Correção Técnica:</strong><br>
                👤 Integrante Responsável: <span style="color: #ff4b4b; font-weight: bold;">[{passo_falho['quem_correto']}]</span><br>
                📦 Equipamento/Ação Correta: <span style="color: #ff4b4b; font-weight: bold;">[{passo_falho['o_que_correto']}]</span><br>
                ℹ️ Justificativa: {passo_falho['motivo']}
            </div>
            """, 
            unsafe_allow_html=True
        )
        if st.button("Reiniciar Treinamento de Segurança 🔄", use_container_width=True, type="primary"):
            resetar_jogo()
            st.rerun()

    # Exibição do Alerta de Segurança Compacto (Imagem pequena ao lado do erro)
    elif st.session_state.erro_procedimento:
        fluxo_ativo = obter_fluxo_nr33() if modulo == "NR-33 (Espaço Confinado)" else obter_fluxo_nr35()
        
        # Grid para colocar imagem pequena ao lado da caixa de erro
        c_alerta_img, c_alerta_txt = st.columns([1, 4])
        with c_alerta_img:
            exibir_imagem_repositorio("Alerta_Seguranca.png", "Alerta SST", largura=75)
        with c_alerta_txt:
        ...
        if st.session_state.etapa_atual >= len(fluxo_seguranca):
            ...
            # Alternância Dinâmica de Telas baseada no preenchimento do Passo 1
            if st.session_state.responsavel_selecionado is None:
                st.markdown("#### 🟥 **PASSO 1:** Clique primeiro no **Responsável** pela tarefa:")
                
                # Renderização da Equipe (Passo 1 ativo)
                ...
                if ocultar_vigia:
                    ...
                    with c_eq1:
                        ...
                        if st.button("Selecionar Supervisor", key="k_sup", use_container_width=True):
                            avaliar_dupla("quem", "Supervisor")
                    with c_eq2:
                        exibir_imagem_repositorio("Entrante.png", "Trabalhador")
                        if st.button("Selecionar Trabalhador", key="k_ent", use_container_width=True):
                            avaliar_dupla("quem", "Entrante")
                    with c_eq3:
                        exibir_imagem_repositorio("Resgate1.png", "Equipe Resgate")
                        if st.button("Selecionar Resgate", key="k_res", use_container_width=True):
                            avaliar_dupla("quem", "Resgate")
                else:
                    ...
                    with c_eq3:
                        ...
                        if st.button("Selecionar Vigia", key="k_vig", use_container_width=True):
                            avaliar_dupla("quem", "Vigia")
                    with c_eq4:
                        exibir_imagem_repositorio("Resgate1.png", "Equipe Resgate")
                        if st.button("Selecionar Resgate", key="k_res", use_container_width=True):
                            avaliar_dupla("quem", "Resgate")
                ...
                # --- SEÇÃO 2: ISOLAMENTO E BLOQUEIO (LOTO) ---
                st.markdown("#### 🔒 2. Isolamento, Bloqueio e Gestão Organizacional")
                m1, m2, m3, m4, m5, m6 = st.columns(6)
                with m1:
                    exibir_imagem_repositorio("Isolamento.png", "Isolamento")
                    if st.button("Isolamento Área", key="k_iso", use_container_width=True):
                        avaliar_dupla("o_que", "Isolamento")
                with m2:
                    exibir_imagem_repositorio("Cadeado.png", "LOTO")
                    if st.button("Cadeado / LOTO", key="k_loto", use_container_width=True):
                        avaliar_dupla("o_que", "LOTO")
                with m3:
                    exibir_imagem_repositorio("Sinalizacao.NaoOpere.png", "Sinalizacao")
                    if st.button("Sinalização LOTO", key="k_sin", use_container_width=True):
                        avaliar_dupla("o_que", "Sinalizacao")
                with m4:
                    exibir_imagem_repositorio("PET.png", "Documentação")
                    if st.button("Assinar PET / PT", key="k_pet", use_container_width=True):
                        avaliar_dupla("o_que", "PET")
                with m5:
                    exibir_imagem_repositorio("Cilindro_Teste_Resposta.png", "Bump Test")
                    if st.button("Acionar Bump Test", key="k_bt", use_container_width=True):
                        avaliar_dupla("o_que", "Teste Resposta")
                with m6:
                    exibir_imagem_repositorio("APR.png", "Análise de Risco")
                    if st.button("Validar APR", key="k_apr", use_container_width=True):
                        avaliar_dupla("o_que", "APR")

                # --- SEÇÃO 3: ATMOSFERA E LINHAS DE VIDA ---
                ...
                with n1:
                    ...
                    if st.button("Ventilação/Purga", key="k_vent", use_container_width=True):
                        avaliar_dupla("o_que", "Ventilacao")
                with n2:
                    exibir_imagem_repositorio("DetectorGas.png", "Detector")
                    if st.button("Medição Gases", key="k_det", use_container_width=True):
                        avaliar_dupla("o_que", "Detector")
                with n3:
                    exibir_imagem_repositorio("Tripe.png", "Tripé")
                    if st.button("Tripé / Roldanas", key="k_tri", use_container_width=True):
                        avaliar_dupla("o_que", "Tripe")
                with n4:
                    exibir_imagem_repositorio("Talabarte_Y.png", "Talabarte Y")
                    if st.button("Acionar Talabarte Y", key="k_ty", use_container_width=True):
                        avaliar_dupla("o_que", "Talabarte_Y")
                with n5:
                    exibir_imagem_repositorio("TravaQuedas.png", "TravaQuedas")
                    if st.button("Acionar Trava-Quedas", key="k_tq", use_container_width=True):
                        avaliar_dupla("o_que", "TravaQuedas")

                # --- SEÇÃO 4: SEGURANÇA INDIVIDUAL ---
                st.markdown("#### 🪖 4. Equipamentos Individuais e Proteção de Trauma")
                o1, o2, o3, o4 = st.columns(4)
                with o1:
                    exibir_imagem_repositorio("Cinto_Seguranca.png", "EPI")
                    if st.button("Equipar Cinto/EPI", key="k_epi", use_container_width=True):
                        avaliar_dupla("o_que", "EPI")
                with o2:
                    exibir_imagem_repositorio("Radio_Comunicacao.png", "HT")
                    if st.button("Rádio HT", key="k_com", use_container_width=True):
                        avaliar_dupla("o_que", "Comunicacao")
                with o3:
                    exibir_imagem_repositorio("Lado_de_fora.png", "Posto Externo")
                    if st.button("Posto Externo (Fora)", key="k_out", use_container_width=True):
                        avaliar_dupla("o_que", "LadoFora")
                with o4:
                    exibir_imagem_repositorio("Fita_AntiTrauma.png", "Fita Alívio")
                    if st.button("Fita Anti-Trauma", key="k_fat", use_container_width=True):
                        avaliar_dupla("o_que", "Fita_AntiTrauma")

                # --- SEÇÃO 5: EMERGÊNCIA TÁTICA FINAL INTEGRADA ---
                if st.session_state.etapa_atual == (len(fluxo_seguranca) - 1):
                    ...
                    with f1:
                        ...
                        if st.button("Respirador Autônomo", key="f_m1", use_container_width=True):
                            avaliar_dupla("o_que", "Respirador")
                    with f2:
                        exibir_imagem_repositorio("Maca_Rígida.png", "Maca Rígida")
                        if st.button("Maca Rígida + Colar", key="f_m2", use_container_width=True):
                            avaliar_dupla("o_que", "MacaRigida")
                    with f3:
                        exibir_imagem_repositorio("Maca_Sked.png", "Maca Sked")
                        if st.button("Maca Sked Envelope", key="f_m3", use_container_width=True):
    ...
    # --- HISTÓRICO DAS AÇÕES CONCLUÍDAS ---

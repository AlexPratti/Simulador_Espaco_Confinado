...
# --- NAVEGAÇÃO PRINCIPAL POR ABAS NATIVAS ---
st.markdown("""
<style>
...
abas = st.tabs(["NR-33 (Espaço Confinado)", "NR-35 (Trabalho em Altura)"])
...
if "aba_atual" not in st.session_state:
    st.session_state.aba_atual = "NR-33"
...
# --- MAPEAMENTO E CONTROLE DE NAVEGAÇÃO DA ABA ATIVA ---
with abas[0]:
    if st.session_state.aba_atual != "NR-33":
        st.session_state.aba_atual = "NR-33"
        resetar_jogo()
        st.rerun()

with abas[1]:
    if st.session_state.aba_atual != "NR-35":
        st.session_state.aba_atual = "NR-35"
        resetar_jogo()
        st.rerun()

aba_ativa = "NR-33 (Espaço Confinado)" if st.session_state.aba_atual == "NR-33" else "NR-35 (Trabalho em Altura)"
...
with col_esquerda:
    ...
    if aba_ativa == "NR-33 (Espaço Confinado)":
        ...
        if st.session_state.servico_selecionado:
            ...
            st.markdown("### 📐 Painel de Engenharia de Queda")
            h_cenario = st.number_input("Altura total do cenário (metros):", min_value=1.0, max_value=50.0, value=4.0, step=0.5)
            dist_topo_anc = st.number_input("Distância do topo até a ancoragem (metros):", min_value=0.0, max_value=10.0, value=1.5, step=0.1)
            zlq_talabarte = st.number_input("ZLQ requerida pelo Talabarte Y (metros):", min_value=1.0, max_value=15.0, value=5.0, step=0.1)
            
            fq_analise = ""
            if "Telhado" in st.session_state.servico_selecionado: fq_analise = "FQ = 2 (Crítico - Nos pés)"
            elif "Fachada" in st.session_state.servico_selecionado: fq_analise = "FQ = 1 (Médio - No peito)"
            else: fq_analise = "FQ < 1 (Ideal - Acima da cabeça)"
            ...
            if zlq_local >= zlq_talabarte:
                st.success("🟢 **ATIVIDADE LIBERADA:** Distância segura!")
                st.session_state.bloqueio_altura = False
            else:
                st.error("🔴 **ATIVIDADE SUSPENSA:** Risco de impacto contra o chão!")
                st.session_state.bloqueio_altura = True
...
with col_direita:
    ...
    if not st.session_state.servico_selecionado or not st.session_state.acidente_selecionado:
    ...
    elif aba_ativa == "NR-35 (Trabalho em Altura)" and st.session_state.get("bloqueio_altura", False):
        st.warning("⚠️ **PAINEL BLOQUEADO:** Corrija os parâmetros de Engenharia de Queda na coluna da esquerda. A atividade está suspensa devido ao risco de colisão.")
        ...
        fluxo_ativo = obter_fluxo_nr33() if aba_ativa == "NR-33 (Espaço Confinado)" else obter_fluxo_nr35()
        ...
        fluxo_seguranca = obter_fluxo_nr33() if aba_ativa == "NR-33 (Espaço Confinado)" else obter_fluxo_nr35()
        ...
        if st.session_state.etapa_atual >= len(fluxo_seguranca):
            ...
            ocultar_vigia = (aba_ativa == "NR-35 (Trabalho em Altura)") and ("Sem Vigia" in st.session_state.servico_selecionado)
...
with col_esquerda:
    ...
    if aba_ativa == "NR-33 (Espaço Confinado)":
        ...
        # ABA DA NR-35: CÁLCULOS TÉCNICOS E ENGENHARIA DE QUEDA
        st.selectbox("1. Escolha a atividade industrial em altura:", servicos_nr35, index=None, placeholder="Selecione...", on_change=resetar_jogo, key="sel_s_35")
        st.session_state.servico_selecionado = st.session_state.sel_s_35
        st.selectbox("2. Escolha o risco à saúde associado:", acidentes_disponiveis, index=None, placeholder="Selecione...", on_change=resetar_jogo, key="sel_a_35")
        st.session_state.acidente_selecionado = st.session_state.sel_a_35
